...
def _get_lcs_cut2(s1, s2, lcs_low_bound=0, bg=None, debug=False):
    ...
    Though, detail of implementation of s1 and s2 object slicing will
    affect the optimal performance.
    ...
    rngc_lcs = [None] * (n+1)
    rngp_lcs = [None] * (n+1)
    rngc = array('i', [0]*(n+1))  # current row
    rngp = array('i', [0]*(n+1))  # previous row
